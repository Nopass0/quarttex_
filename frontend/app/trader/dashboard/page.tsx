...
export default function TraderDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const totalAmount = filteredDeals.reduce((sum: number, deal: any) => sum + deal.amount, 0)
      ...
      setFinanceStats({
        deals: { amount: totalAmount, amountRub: totalAmountRub, count: totalDeals },
      ...
      <AuthLayout variant="trader">
        <div className="space-y-6">
              ...
              <Card className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-semibold">{financeStats.deals.count}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Объем</p>
                    <p className="text-lg font-medium">{financeStats.deals.amount.toFixed(2)} USDT</p>
                    <p className="text-sm text-gray-500">≈ {financeStats.deals.amountRub.toFixed(0)} ₽</p>
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-semibold text-[#006039]">{financeStats.profit.amount.toFixed(2)} USDT</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">В рублях</p>
                    <p className="text-lg font-medium">≈ {financeStats.profit.amountRub.toFixed(0)} ₽</p>
                  </div>
                </div>
              </Card>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Devices */}
            <section>
              <Link href="/trader/devices" className="flex items-center justify-between mb-4 group">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Open Disputes */}
            <section>
              <Link href="/trader/disputes" className="flex items-center justify-between mb-4 group">
              ...
              <div className="space-y-1">
                {disputes.length === 0 ? (
                  ...
                  disputes.map((dispute) => (
                    <Link key={dispute.id} href={`/trader/disputes/${dispute.id}`}>
                      <div className="border-b border-gray-100 p-3 hover:bg-gray-50 transition-colors cursor-pointer">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-sm font-medium">Спор #{dispute.id}</p>
            ...
            {/* Recent Deals */}
            <section>
              <Link href="/trader/deals" className="flex items-center justify-between mb-4 group">
                    ...
                    <Link key={deal.id} href={`/trader/deals?id=${deal.id}`}>
                      <div className="border-b border-gray-100 p-3 hover:bg-gray-50 transition-colors cursor-pointer">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <p className="text-sm font-medium">{deal.amount.toFixed(2)} USDT</p>
                            <p className="text-xs text-gray-500">{(deal.amount * 100).toFixed(0)} RUB</p>
                          </div>
                        </div>
                      </div>
                    </Link>
