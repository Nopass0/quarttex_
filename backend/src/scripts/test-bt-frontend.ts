...
async function testBtFrontend() {
  try {
    ...
    // Генерируем JWT токен для трейдера
    const JWT_SECRET = process.env.JWT_SECRET || 'your-secret-key';
    const token = jwt.sign(
      { userId: trader.id, role: 'TRADER' },
      JWT_SECRET,
      { expiresIn: '24h' }
    );

    console.log("\n=== Generated Trader Token ===");
    console.log(`Token: ${token}`);
    console.log("\nYou can use this token to test the API endpoint manually.");
    console.log("\nTo test in browser console:");
    console.log(`localStorage.setItem('trader-token', '${token}');`);
    console.log("Then reload the page.");

    // Теперь проверим API эндпоинт с токеном
