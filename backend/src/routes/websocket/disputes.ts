...
const clients = new Map<string, DisputeWsClient>();

export const disputeWebSocketRoutes = new Elysia({ prefix: "/ws" })
  .use(
    jwt({
      name: "jwt",
  ...
  .ws("/disputes", {
    async open(ws) {
      console.log("Dispute WebSocket client connected");
    ...
    close(ws) {
      ...
      console.log("Dispute WebSocket client disconnected");
    },
  });
