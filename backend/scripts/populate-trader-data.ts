...
async function populateTraderData() {
  ...
  try {
    ...
    for (const trader of traders) {
      ...
      for (let i = 0; i < 5; i++) {
        const device = await db.device.create({
          data: {
            ...
            model: deviceNames[i],
            imei: `IMEI${String(i).padStart(15, '0')}`,
            androidId: i % 2 === 0 ? `ANDROID${String(i).padStart(8, '0')}` : null,
            idfv: i % 2 === 1 ? `IDFV${String(i).padStart(16, '0')}` : null,
            totalStorageGb: 128 + i * 64,
            freeStorageGb: 64 + i * 32,
            totalRamGb: 8 + i * 2,
            freeRamGb: 4 + i,
            batteryLevel: 80 - i * 10,
            isCharging: i % 2 === 0,
            cpuUsage: 20 + i * 5,
            temperature: 35 + i,
            isRooted: false,
            hasVpn: false,
            appVersion: '1.0.0',
            systemVersion: i % 2 === 0 ? 'Android 13' : 'iOS 16',
            language: 'ru',
            timezone: 'Europe/Moscow',
            country: 'RU',
            carrier: ['MTS', 'Beeline', 'Megafon', 'Tele2', 'Yota'][i],
      ...
      // 5. Create Transactions (Deals)
      console.log('💰 Creating transactions...');
      const statuses = [Status.CREATED, Status.IN_PROGRESS, Status.READY, Status.FAILED, Status.EXPIRED];
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const transaction = await db.transaction.create({
          data: {
            ...
            acceptedAt: status === Status.IN_PROGRESS || status === Status.READY ? new Date() : null,
            ...
            metadata: {
              ...
              processingTime: status === Status.READY ? Math.floor(Math.random() * 300) + 60 : null
            }
          }
        });
    ...
    console.log('- 20 transactions per trader');
